...
QUESTION_BANK = [
...

    {
        ...
        "options": [
            "A) Raw Text → Normalization → Tokenization",
            "B) Tokenization → Raw Text → Normalization",
            "C) Lemmatization → Raw Text → Tokenization",
            "D) Stemming → Raw Text → Stop Words"
        ],
        "answer_index": 0,
...
def render_report_section():
    ...
    st.write(
        ...
        f"{len(st.session_state.get('current_quiz', []))}"
    )
    ...
    pdf_bytes = generate_pdf_report(
        ...
        quiz_total=len(st.session_state.get("current_quiz", [])),
